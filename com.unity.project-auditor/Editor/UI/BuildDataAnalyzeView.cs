                ...
                using (new EditorGUILayout.HorizontalScope())
                {
                    ...
                    using (new EditorGUILayout.VerticalScope())
                    {
                        ...
                        if (doRun)
                        {
                            ...
                            if (view == null || !k_BuildDataCategories.Contains(view.Desc.Category))
                            {
#if PA_DRAW_TABS_VERTICALLY
                                m_ProjectAuditorWindow.ViewSelectionTreeView.Reload();
                                m_ProjectAuditorWindow.ViewSelectionTreeView.SelectItemByCategory(k_BuildDataCategories[0]);
#endif

                                m_ViewManager.ChangeView(k_BuildDataCategories[0]);
                            }
                        }
                    }
