...
//        }
    ...
    public class AssemblySelectionWindow : EditorWindow
    {
        ...
        TreeViewState m_AssemblyTreeViewState;
        MultiColumnHeaderState m_AssemblyMulticolumnHeaderState;
        ...
        static public AssemblySelectionWindow Open(float screenX, float screenY, ProjectAuditorWindow projectAuditorWindow, TreeViewSelection assemblySelection, string[] assemblyNames)
        {
            ...
            window.SetData(projectAuditorWindow, assemblySelection, assemblyNames);
            window.Show();
        ...
        void CreateTable(ProjectAuditorWindow projectAuditorWindow, string[] assemblyNames, TreeViewSelection assemblySelection)
        {
            if (m_AssemblyTreeViewState == null)
                m_AssemblyTreeViewState = new TreeViewState();

            m_AssemblyMulticolumnHeaderState = AssemblyTable.CreateDefaultMultiColumnHeaderState(700);

            var multiColumnHeader = new MultiColumnHeader(m_AssemblyMulticolumnHeaderState);
            multiColumnHeader.SetSorting((int)AssemblyTable.MyColumns.AssemblyName, true);
            multiColumnHeader.ResizeToFit();
            m_AssemblyTable = new AssemblyTable(m_AssemblyTreeViewState, multiColumnHeader, assemblyNames, assemblySelection);
        }

        void SetData(ProjectAuditorWindow projectAuditorWindow, TreeViewSelection assemblySelection, string[] assemblyNames)
        {
            m_ProjectAuditorWindow = projectAuditorWindow;
            CreateTable(projectAuditorWindow, assemblyNames, assemblySelection);
        }
